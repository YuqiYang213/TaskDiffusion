...
def train_phase(p, args, train_loader, test_dataloader, model, criterion, optimizer, scheduler, epoch, tb_writer, tb_writer_test, iter_count):
    ...
    for i, cpu_batch in enumerate(tqdm(train_loader)):
        ...
        images = batch['image']
        if p.with_aux:
            output, output_aux = model(images, batch, if_eval=False, aux_forward=p.with_aux)
            iter_count += 1
            

            loss_dict = criterion(output, batch, tasks=p.TASKS.NAMES)
            loss_dict_aux = criterion(output_aux, batch, tasks=p.TASKS.NAMES)
            # get learning rate
            lr = scheduler.get_lr()
            ...
            # Backward
            optimizer.zero_grad()
            loss_all = (loss_dict['total'] + loss_dict_aux['total'] * p.aux_weight)
            loss_all.backward()
        else:
            output = model(images, batch, if_eval=False, aux_forward=p.with_aux)
            iter_count += 1
