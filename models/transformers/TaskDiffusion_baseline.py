...
class MLoRE(nn.Module):
    ...
    def forward(self, x):
        ...
        last_feat = {task: [] for task in all_tasks}
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx + 1 in self.select_list: 
                ...
                decode_feat = rearrange(x, 'b (h w) c -> b c h w', h=self.resolution[0], w=self.resolution[1])
                decode_feat = self.fea_fuse[il](decode_feat)
                for task in all_tasks:
                    out_feat[task].append(decode_feat.unsqueeze(1))
        ...
        for task in all_tasks:
            out_feat[task].append(decode_feat.unsqueeze(1))
            last_feat[task] = torch.cat(out_feat[task], dim=1) # B x 4 x C x H x W
            last_feat[task] = rearrange(last_feat[task], 'b l c h w -> b (l c) h w')

        # the dimension should be B x 4C x H x W
...
def _create_TaskDiffusion(variant, pretrained=False, default_cfg=None, **kwargs):
...
def TaskDiffusion_vit_large_patch16_384(pretrained=False, **kwargs):
    ...
    model = _create_TaskDiffusion('vit_large_patch16_384', pretrained=pretrained, **model_kwargs)
    return model

def TaskDiffusion_vit_base_patch16_384(pretrained=False, **kwargs):
    ...
    model = _create_TaskDiffusion('vit_base_patch16_384', pretrained=pretrained, **model_kwargs)
    return model

def TaskDiffusion_vit_small_patch16_384(pretrained=False, **kwargs):
    ...
    model = _create_TaskDiffusion('vit_small_patch16_384', pretrained=pretrained, **model_kwargs)
    return model
