...
class MOEBlock(nn.Module):
    def __init__(self, tasks, final_embed_dim, im_size, rank_list, topk, kernel_size=1, with_feat=False):
        super().__init__()
        self.num_lora = 15
        # self.p = p
        ...
        for task in tasks:
            ...
            self.conv2[task] = LoraBlock(final_embed_dim, final_embed_dim, kernel_size=kernel_size, rank=72)

            self.bn[task] = nn.BatchNorm2d(final_embed_dim)
            self.bn_all[task] = nn.BatchNorm2d(final_embed_dim)

        self.router_1 = nn.ModuleDict() 
        self.pre_softmax = False
        self.desert_k = len(rank_list) - topk
        for task in tasks:
    ...
    def forward(self, x, task, route_feat_in=None):
        ...
        out = out_ori
        n, c, h, w = out.shape
        route_feat = self.router_1[task][0](out, route_feat_in) # torch.Size([2, 384, 1, 1])
        prob_all = self.router_1[task][1](route_feat).unsqueeze(2) # torch.Size([2, 31, 1, 1, 1])
        prob_lora, prob_mix = prob_all[:, :self.num_lora * 2], prob_all[:, self.num_lora * 2:] # torch.Size([2, 30, 1, 1, 1]) torch.Size([2, 1, 1, 1, 1])
        ...
        if self.pre_softmax:
            ...
            route_1_indice = torch.topk(route_1_raw, self.desert_k, dim=1, largest=False)[1] # torch.Size([2, 6, 1, 1, 1]) # caculate gate value
            for j in range(n):
        ...
        lora_out_1 = []
        for i in range(self.num_lora):
            lora_out_1.append(self.lora_list_1[i](out).unsqueeze(1)) # n, 1, c, h, w torch.Size([2, 1, 768, 28, 36])
        lora_out_1 = torch.cat(lora_out_1, dim=1)
        lora_out_1 = torch.sum(lora_out_1 * route_1, dim=1)
        out = self.bn_all[task](lora_out_1) + self.conv2[task](out) * prob_mix[:, 0] + self.share_conv(out.detach()) # conv2 as low rank
...
class FPN_withtime(nn.Module):
    def __init__(self, embed_dim, time_dim, tasks, topk, rank_list, num_layer=4, num_conv_layer=2, spatial_size=1024):
        super().__init__()
        ...
        for i in range(num_layer):
            ...
            self.feat_task.append(nn.ModuleList())
            for j in range(2):
                self.feat_task[i].append(MOEBlock(tasks, embed_dim, spatial_size, rank_list=rank_list, topk=topk, kernel_size=3))
    ...
    def forward(self, x, t, mask=None, x_ori=None):
        ...
        route_prob_2 = None
        if mask is None:
            route_prob_1 = [{task:None for task in self.tasks} for i in range(self.num_layer)]
            route_prob_2 = [{task:None for task in self.tasks} for i in range(self.num_layer)]
            ...
            for i in range(self.num_layer):
                ...
                for task in self.tasks:
                    mask[i][task], _, route_prob_1[i][task] = self.feat_task[i][0](x_ori[:, i], task)
                    mask[i][task], _, route_prob_2[i][task] = self.feat_task[i][1](mask[i][task], task)
        ...
        return x_last, mask, route_prob_1, route_prob_2
    


class Fpn_Aux(nn.Module):
    def __init__(self, embed_dim, num_layer):
        super().__init__()
        self.task_mask = nn.Sequential(
                    nn.Conv2d(embed_dim * num_layer, embed_dim, kernel_size=1),
                    nn.BatchNorm2d(embed_dim), nn.GELU(),  
                    nn.Conv2d(embed_dim, num_layer, kernel_size=3, padding=1))
        self.num_layer = num_layer

    def forward(self, x):
        n, c, h, w = x.shape
        mask_now = self.task_mask(x)
        mask_now = torch.softmax(mask_now, dim=1)
        x = x.reshape(n, 4, c // 4, h, w)
        out = 0
        for il in range(self.num_layer):
            out = out + mask_now[:, il].unsqueeze(1) * x[:, il]
        return out
...
class Crosstask_diffusion_decoder_mlore(nn.Module):
    ...
    def __init__(self,
        ...
        super(Crosstask_diffusion_decoder_mlore, self).__init__()
        ...
        if with_auxiliary_head:
            ...
            for task in all_tasks:
                self.aux_heads[task] = nn.Sequential(Fpn_Aux(embed_dim, num_layer), 
        ...
        # add fpn layer
        self.fpn_encoder = FPN_withtime(embed_dim, time_dim, all_tasks, spatial_size=p.spatial_size, rank_list=p.rank_list, topk=p.topk)
    ...
    def forward_train(self, x, gt_semantic_seg):
        ...
        logits, feat_aux, info = self._decode_head_forward(feat, input_times, x_ori=x['semseg'])
    ...
    def _decode_head_forward(self, x, t, x_ori=None, mask=None, if_up=True):
        ...
        all_task = self.p.TASKS.NAMES
        x, mask, route_prob_1, route_prob_2 = self.fpn_encoder(x, t, x_ori=x_ori, mask=mask)
    ...
    def ddim_sample(self, x):
        ...
        for idx, (times_now, times_next) in enumerate(time_pairs):
            ...
            mask_logits, mask_cur, _ = self._decode_head_forward(feat, input_times, x_ori=x['semseg'], mask=mask_cur, if_up=(idx == (len(time_pairs) - 1)))  # a dictionary
            mask_pred_all = []
            gt_down_split = []
            for task in all_task:
                if all_classes[task] > 0:
